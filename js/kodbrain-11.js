(() => {
  // =========================================================
  // DI / KODUX · ORB UNIFIED CORE (FULL MERGED)
  // =========================================================
  ...
  // 🔒 FLAGS DE CONTROLE PARA UI SECUNDÁRIA
  DI.flags = {
    lockSmallUI: true,     // impede sobrescrita de #smallText
    lockBadges: true,      // impede sobrescrita de #actBadge
    lockIdent: true        // impede sobrescrita de #smallIdent
  };

  const getEl = (sel) => document.querySelector(sel);
  const setText = (sel, value) => { const el = getEl(sel); if (el) el.textContent = value; };
  ...
  function injectOrbStyles() {
    ...
    style.textContent = `
      ...
      #main-orb[data-context="speaking"] .orb-core {
        animation: orbSpin 2s linear infinite, orbPulse 0.5s ease-in-out infinite alternate;
      }
      ...
      @keyframes orbPulse { from { transform: scale(1); } to { transform: scale(1.12); } }
    `;
    document.head.appendChild(style);
  }

  // ---------------------------------------------------------
  // DI · COMPUTE / ROOT VARS
  // ---------------------------------------------------------
  ...
  function di_renderOrb(selector, name, size = 36) {
  ...
  const Brain = {
    ...
    safeName() {
      return (
        getEl('#inputUser')?.value?.trim() ||
        getEl('#infodoseNameInput')?.value?.trim() ||
    ...
    renderOrb(targetSel, name, size = 36) {
      const el = getEl(targetSel);
      if (!el) return;
    ...
    syncIdentity() {
      ...
      // 🔒 UI SECUNDÁRIA – RESPEITANDO FLAGS DE CONTROLE
    ...
    syncMode() {
      const mode = this.get('fusion_os_ui_state', {})?.mode ||
                   this.get('bodyPlayerMode') ||
                   document.body.dataset.mode ||
                   'player';

      const bodyPlayer = getEl('#bodyPlayer');
      if (bodyPlayer) bodyPlayer.dataset.mode = mode;
    },

    syncFusionState() {
      const mainCard = getEl('#mainCard');
      const symbolBar = getEl('#symbolBar');
    ...
    bindInputs() {
      ...
      const saveBtn = getEl('#saveSystemBtn');
  ...
  // ---------------------------------------------------------
  // EXPORTAÇÕES PÚBLICAS (DI & GLOBAL)
  // ---------------------------------------------------------
