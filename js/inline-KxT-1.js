// --- Adapter: registra App no di_
di_.register('AppShell', function(di){
  // Insere integração reativa com di_.state
  function onStateChange(payload){
    // payload: { key, val, field }
    if(payload.field === 'apiKey' && document.getElementById('apiKeyInput')){
      document.getElementById('apiKeyInput').value = payload.val || '';
    }
    if(payload.field === 'userName' && document.getElementById('inputUserId')){
      document.getElementById('inputUserId').value = payload.val || '';
      document.getElementById('usernameDisplay') && (document.getElementById('usernameDisplay').textContent = payload.val || '');
    }
    // adicione mais bindings conforme necessário
  }
  di.bus.on('state-change', onStateChange);
  // retorna API do módulo (opcional)
  return { name: 'AppShell', dispose(){ di.bus.off('state-change', onStateChange); } };
});
/* =========================================================
