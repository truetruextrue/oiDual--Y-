...
/**
 * KOB TTS ENGINE + ARCHETYPE INTEGRATION
 * Conecta o Dock aos perfis de voz do Voice-Map-colors-Arch.js
 */
...
class KobTtsEngine {
  ...
  init() {
    // Aguarda as vozes do sistema carregarem
    if (this.synth.onvoiceschanged !== undefined) {
      this.synth.onvoiceschanged = () => { this._voicesLoaded = true; };
    }

    // Escuta os eventos do componente UI (Dock)
    ...
    // Escuta mudanças de arquétipo para atualizar cores se necessário
    window.addEventListener('kob-tts:archetype-change', (e) => {
      console.log('Arquétipo alterado para:', e.detail.id);
    });
  }

  /**
   * Obtém as configurações do arquétipo ativo no sistema KOBLLUX
   */
  _getActiveArch() {
    if (window.KOBLLUXVoices) {
      return window.KOBLLUXVoices.getActiveArchetype();
    }
    // Fallback caso o script de cores não tenha carregado
    return { name: 'Padrão', rate: 1, pitch: 1, voice: 'Google português do Brasil' };
  }

  /**
   * Tenta encontrar a voz mais adequada no navegador
   */
  _getBestVoice(preferredName) {
    const voices = this.synth.getVoices();
    if (!voices.length) return null;

    // 1. Tenta correspondência exata ou parcial pelo nome no arquétipo
    if (preferredName) {
      const match = voices.find(v => v.name.toLowerCase().includes(preferredName.toLowerCase()));
      if (match) return match;
    }

    // 2. Fallback para Português
    return voices.find(v => v.lang.includes('pt')) || voices[0];
  }

  speak() {
    // Se estiver apenas pausado, retoma o áudio
    ...
    this.stop(); // Garante que não há fofoca cruzada

    // Seleção de conteúdo: prioriza blocos de resposta, depois main, depois body
    const content = document.querySelector('.response-block:last-of-type') || 
    ...
    // Aplica a "Personalidade" do Arquétipo
    utterance.voice = this._getBestVoice(arch.voice);
    ...
    utterance.onstart = () => {
      this.isPaused = false;
      console.log(`%c KOBLLUX falando como: ${arch.name}`, `color: ${arch.colorMain}`);
    };
...
// Inicializa o motor
if (!window.kobTtsEngineInstance) {
  window.kobTtsEngineInstance = new KobTtsEngine();
}
