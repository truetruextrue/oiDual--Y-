...
---------- */
    ...
    /* -----------------------------
   bind click-to-speak inside iframe
   ----------------------------- */
...
function bindIframeClickToSpeak() {
  try {
    ...
    if (!doc || doc === __kobBoundDoc) return;
    ...
    const onSelection = () => {
      const s = String(doc.getSelection ? doc.getSelection() : window.getSelection());
      if (!s || !s.trim()) return;

      const arch = ARCHETYPES[state.archIdx] || ARCHETYPES[0];
      const txt = sanitize(s.trim());
      ...
      try {
        if (window.KOBLLUX_VOICE_ENGINE && typeof window.KOBLLUX_VOICE_ENGINE.speakWithCurrentArchetype === 'function') {
          ...
          const ok = window.KOBLLUX_VOICE_ENGINE.speakWithCurrentArchetype(txt, {});
          if (ok) return;
        }
      } catch (e) {
        console.warn('engine selection speak failed', e);
      }
    ...
    doc.addEventListener('pointerup', onSelection, { passive: true });
