// kob-glue-dh10.js — versão simplificada com contador no hudStatus
(function(){
  'use strict';
  ...
  const BTN_PLAY = $('#btn-play'); // pode não existir mais, mas mantemos referência
  const BTN_NEXT = $('#btn-next');
  const BTN_PREV = $('#btn-prev');
  const BTN_ARCH = $('#btn-arch');

  /* -----------------------------
     Archetypes (lista completa)
     ----------------------------- */
  const ARCHETYPES = [
  ...
  /* -----------------------------
     Symbol bar handler (apenas botões relevantes)
     ----------------------------- */
  (function attachSymbolBarHandler(){
    ...
    bar.addEventListener('click', (ev) => {
      ...
      // URL open buttons (caso ainda existam no HTML, mas removemos)
      ...
      switch(bid){
        ...
        case 'tts-grid':
          ...
          break;
        case 'toggleBtn':
          // já tratado no setupHUD
          break;
        default:
          // outros botões (play/next/prev/arch) não existem mais, ignorar
          break;
      }
    }, { passive: true });
  })();
  ...
  /* -----------------------------
     updateArchetype (mantida, mas sem alterar hudStatus)
     ----------------------------- */
  function updateArchetype(idx){
    ...
    try{
      ...
      // NÃO alteramos hudStatus aqui – ele mostra apenas o contador
    }catch(e){
  ...
  // setStatus agora atualiza o hudStatus com o contador
  function setStatus(){
    // Atualiza elemento #tts-status se existir (legado)
    const el = $('#tts-status');
    if (el) {
    ...
    // Atualiza hudStatus com o contador
    if (hudStatus) {
      if (!state.blocks.length) hudStatus.textContent = '0/0';
      else hudStatus.textContent = `${Math.min(state.currentBlockIdx+1, state.blocks.length)}/${state.blocks.length}`;
    }
  }
  ...
  /* -----------------------------
     speakCurrent() — usa engine quando disponível
     ----------------------------- */
  ...
  /* -----------------------------
     Outros listeners (mantidos por compatibilidade)
     ----------------------------- */
...
`;
  ...
  console.log('KOBLLUX glue init (simplificado) ✓');
