(function(){
  ...
  // 🔑 ativa arquétipo atual no engine
  if(window.KOBLLUX_VOICE_ENGINE){
    KOBLLUX_VOICE_ENGINE.activateArchetype(
      ARCHETYPES[state.archIdx].id
    );
  }

  // 🔑 delega a fala (morph + voz + tema)
  const ok = window.KOBLLUX_VOICE_ENGINE &&
             KOBLLUX_VOICE_ENGINE.speakWithMorph(txt, {
               onStart(){
                 showOutlineFor(el);
                 setStatus();
               },
  ...
  if(!ok){
    console.warn('VoiceEngine indisponível, fallback monólito');
    synth.cancel();
