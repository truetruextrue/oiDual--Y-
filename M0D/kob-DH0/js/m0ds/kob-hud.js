/* kob-hud.js — extracted HUD JS (drop-in to replace inline script) */
(function () {
  'use strict';
  if (window.__KOB_HUD_EXTRACTED_INIT__) { console.log('KOB HUD already init'); return; }
  window.__KOB_HUD_EXTRACTED_INIT__ = true;
  ...
  const frame = document.querySelector('#content-frame, #frame');
  ...
  (function setupHUD(){
    ...
    bar.addEventListener('pointerdown', e => {
      if(e.target.closest && e.target.closest('.symbol-button')) return;
    ...
    if(toggleBtn){
      toggleBtn.addEventListener('click', ()=>{
        ...
        try{ localStorage.setItem('kob_collapsed', state.isCollapsed); }catch(e){}
    ...
    bar.addEventListener('click', (ev) => {
      ...
      switch(bid){
        ...
        case 'btn-prev':
          ...
          break;
        case 'btn-arch':
          if(!callTTS('cycleArchetype')) {
            if(window.KOBLLUX && window.KOBLLUX.updateArchetype) {
              window.KOBLLUX.updateArchetype((window.KOBLLUX.state.archIdx||0)+1);
            }
          }
          break;
        default:
  ...
  function hexToRgba(hex,a){ const c = String(hex||'#000').replace('#',''); const r = parseInt(c.slice(0,2),16)||0, g = parseInt(c.slice(2,4),16)||0, b = parseInt(c.slice(4,6),16)||0; return `rgba(${r},${g},${b},${a})`; }

  // Apply archetype theme (defensive)
  ...
  function scanBlocks(){
    try{
      if(frame && frame.contentWindow){
        ...
        const nodes = Array.from(doc.querySelectorAll(sel)).filter(n=> (n.innerText||'').trim().length > 0);
        if(nodes.length){ state.blocks = nodes; state.currentBlockIdx = 0; return; }
      }
    }catch(e){}
    const localNodes = Array.from(root.querySelectorAll('h1,h2,h3,p,li,blockquote,pre,td,th')).filter(n=> (n.innerText||'').trim().length > 0);
  ...
  $('#tts-sel') && $('#tts-sel').addEventListener('click', () => {
    ...
    const u = new SpeechSynthesisUtterance(String(s).trim());
    const voice = findVoiceByNamePart(arch.voice);
    if (voice) u.voice = voice;
    if (arch.lang) u.lang = arch.lang;
    u.rate  = arch.rate;
  ...
  console.log('KOB HUD extracted init');
  toast('KOBLLUX HUD pronto ✓', 900);
