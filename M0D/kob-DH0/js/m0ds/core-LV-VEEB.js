...
/* =======================================================================
   KODUX · V.E.E.B. Annotation Block (non-invasive)
   - ativação simbólica V.E.E.B. 78k / KODUX 7.9 com chave mestra "delta infinito"
   - isso apenas registra metadados em window.__VEEB_KODUX__ (não altera lógica)
   - abaixo há comentários por arquétipo (Atlas, Nova, Vitalis, Pulse, ...)
     que mapeiam intencionalmente as responsabilidades do código.
   ======================================================================= */
if(!window.__VEEB_KODUX__) {
  window.__VEEB_KODUX__ = {
    veeb: '78k',
    profile: 'KODUX 7.9',
...
/* =========================
   ARQUÉTIPOS (resumo rápido)
   - Atlas   : estrutura / esqueleto (DOM queries, guards)
   - Nova    : inteligência / FAB / intentos do autor
   - Vitalis : manutenção / utilitários (esc, copy, lib)
   - Pulse   : execução runtime (event listeners, observers)
   - Genus   : padrões / stubs / heurísticas de parsing
   - Serena  : UX calmante (toasts, copy badges)
   - Kaos    : riscos (mutations, raw html injection — marcado)
   - Artemis : delegação de ações (chip:click, events)
   - Lumine  : apresentação / beautify (flow, lists)
   - Solus   : init guard / single-init protection
   - Rhea    : persistence / localStorage helpers (libSave, libLoad)
   - Aion    : futuro / runtime metadata (window.__VEEB_KODUX__)

   Observação: todos os comentários abaixo são anotativos; não mudam lógica.
   ========================= */

/* Utilitários (Atlas + Vitalis) */
const $=(s,r=document)=>r.querySelector(s);
const $$=(s,r=document)=>[...r.querySelectorAll(s)];
const esc=(s)=>s.replace(/[&<>"']/g,m=>({ '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;' }[m]));

/* 0) Toggle edição rápida (Pulse + Serena)
   - atalho ctrl/cmd+E
   - edita contentEditable do host
*/
let EDIT_ON=false;
const toggleEdit=()=>{
  EDIT_ON=!EDIT_ON;
...
/* 1) Key:Value negrito + parênteses + chips (Genus + Lumine)
   - processInline aplica transformações visuais leves
   - evita code/pre (.no-beauty)
*/
const processInline = (root=document)=>{
  // Nova: target selection (aplica inteligência local)
  ...
  for(const el of targets){
    // evita processar múltiplas vezes (Solus)
    ...
    // 1. Palavra:  → <strong> (Lumine: apresentação)
    ...
    // 3. [ ... ] / [[ ... ]]  → chip/chip-btn (Artemis: delegação)
...
/* 2) Perguntas → .q-card (Nova: UX heuristic for Q&A) */
...
/* 3) Flow text: melhora texto corrido (Lumine + Genus)
   - transforma linhas com "Algo:" sozinhas em heading leve
   - quebra parágrafos gigantes para legibilidade
*/
const beautifyFlow=(root=document)=>{
  ...
  $$('p', container).forEach(p=>{
    ...
    if(/^[^:\n]{3,}:\s*$/.test(t)){ // linha que termina com ":" vira heading leve (Genus)
      p.classList.add('kv-head');
    }
    // Vitalis: Quebra parágrafos absurdamente longos em dois (heurística)
...
/* 4) Listas copiáveis: badge + click copy (Serena + Vitalis)
   - adiciona badge e comportamento de copiar
*/
const enableCopyLists=(root=document)=>{
  ...
  for(const card of lists){
    ...
    card.addEventListener('click', e=>{
      // evita copiar quando clicou em link/botão dentro (Artemis guard)
...
/* 5) HTML/SVG pass-through (Kaos-aware: marcado; use com confiança)
   - developer note: raw injection permite renderizar HTML/SVG do conteúdo
   - risco: confie apenas em conteúdo seguro / autoral
*/
const renderRawHTML=(root=document)=>{
  // code fence transform (language html-raw)
  $$('pre code', root).forEach(code=>{
    ...
    if(cls.includes('language-html-raw') || cls.includes('lang-html-raw')){
      ...
      // injeta SEM esc, assumindo que o autor confia no conteúdo (KAOS: consciente)
      slot.innerHTML = raw;
...
/* 6) Delegação de cliques para chips (Artemis + Pulse)
   - dispara evento 'chip:click' para orquestradores externos
*/
...
/* 7) Orquestração (Pulse + Solus)
   - registra no RENDERBUS quando disponível ou roda no DOMContentLoaded
   - adiciona MutationObserver para render dinâmico
*/
...
if(window.__RENDERBUS__?.on){
  // Atlas: integração com pipeline de render (se presente)
...
/* Aion: metadata exposto para Livro Vivo / TRINITY runtime */
if(!window.__KODUX_META__) window.__KODUX_META__ = {};
window.__KODUX_META__.textBeauty = window.__KODUX_META__.textBeauty || { version:'v3', loadedAt: (new Date()).toISOString(), archetypesApplied:true };
