  ...
  <div id="kxtsk-shell">
   ...
   /* PATCH: HTML por slot + loader de session + vínculo com seus botões */
const DEFAULT_SESSION_URL = "https://kodux78k.github.io/oiDual-H0/DH0-10.html";
const sectionStoreKey = "kx_section_store_v1";

let sectionStore = JSON.parse(localStorage.getItem(sectionStoreKey) || "{}");
...
function getSlotData(row, col) {
  ...
  return sectionStore[key] || {
    html: `<div class="inner"><div class="wrap stack"><div class="blank-slot">Seção ${row * 3 + col + 1} · cole seu HTML aqui</div></div></div>`,
...
function setSlotData(row, col, data) {
  const key = slotKey(row, col);
  sectionStore[key] = {
    html: data.html ?? getSlotData(row, col).html,
    url: data.url ?? getSlotData(row, col).url
  };
  localStorage.setItem(sectionStoreKey, JSON.stringify(sectionStore));
}
...
function openSectionEditor(row, col) {
  ...
  document.getElementById("sectionEditor").setAttribute("aria-hidden", "false");
}

function closeSectionEditor() {
  document.getElementById("sectionEditor").classList.add("hidden");
  document.getElementById("sectionEditor").setAttribute("aria-hidden", "true");
}
...
/* sobrescreve/estende o buildGrid para tornar cada seção clicável */
const _buildGrid = buildGrid;
buildGrid = function () {
  grid.innerHTML = "";
...
function initSessions() {
  // clica em qualquer botao com data-url e abre no iframe
  document.addEventListener("click", (e) => {
    const btn = e.target.closest(".di_icons-btn[data-url], .symbol-button[data-url], [data-url].open-session");
    if (!btn) return;
  ...
  document.getElementById("sectionEditor").addEventListener("click", (e) => {
